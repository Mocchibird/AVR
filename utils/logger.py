...
def log_inference_figure(ori_time_sig, pred_time_sig, metrics, save_dir=None):
    ...
    ax.set_ylim(-np.max(np.abs(ori_time_sig))*1, np.max(np.abs(ori_time_sig))*1)
    ...
    plt.text(text_pos_x, 0.50 - text_pos_y, f"Angle err: {metrics['Angle']:.2f}", transform=plt.gca().transAxes, verticalalignment='top', fontsize=font_size)
    plt.text(text_pos_x, 0.44 - text_pos_y, f"Amp. err: {metrics['Amplitude']:.3f}", transform=plt.gca().transAxes, verticalalignment='top', fontsize=font_size)
    plt.text(text_pos_x, 0.38 - text_pos_y, f"Env. err: {metrics['Envelope']:.3f}", transform=plt.gca().transAxes, verticalalignment='top', fontsize=font_size)
    plt.text(text_pos_x, 0.32 - text_pos_y, f"T60 err: {metrics['T60'] * 100:.2f}%", transform=plt.gca().transAxes, verticalalignment='top', fontsize=font_size)
    plt.text(text_pos_x, 0.26 - text_pos_y, f"C50 err: {metrics['C50']:.2f} db", transform=plt.gca().transAxes, verticalalignment='top', fontsize=font_size)
    plt.text(text_pos_x, 0.20 - text_pos_y, f"EDT err: {metrics['EDT']:.3f} s", transform=plt.gca().transAxes, verticalalignment='top', fontsize=font_size)
    ...
    if save_dir is not None:
        plt.savefig(save_dir, dpi=300, pad_inches=0)  # Change the path and file name as needed
    plt.close("all")
...
def plot_and_save_figure(pred_sig, ori_sig, pred_time, ori_time, position_rx, position_tx, mode_set, save_path):
    ...
    plt.plot(np.real(ori_sig.type(torch.complex64).flatten().cpu().numpy()), alpha=0.5)

    plt.subplot(234)
    plt.title("Imaginary")
    plt.plot(np.imag(pred_sig.detach().cpu().numpy().flatten()))
    plt.plot(np.imag(ori_sig.type(torch.complex64).flatten().cpu().numpy()), alpha=0.5)
    ...
    plt.plot(np.angle(ori_sig.type(torch.complex64).flatten().cpu().numpy()), alpha=0.5)
    plt.tight_layout()
